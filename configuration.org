...
#+begin_src emacs-lisp :results none
  (use-package flycheck
    ...
    :config 
      ...
      (define-fringe-bitmap 'flycheck-fringe-bitmap-continuation double-line)
...
(defun +agenda/show (span)
  (-let* (((month day year) (calendar-current-date 3))
          (three-days-from-now (format "%d-%d-%02d" year month day))
...
(defun +org/property-value (property)
  (first (mapcar
      ...
      (org-element-map
          ...
          'keyword
          (lambda (el) (when (string-match "TITLE" (org-element-property :key el)) el))))))
#+end_src
* Configure =flyspell=
#+begin_src emacs-lisp :results none 
...
(defconst image-directory "./images/")
...
(defun my-org-screenshot-take (width)
  ...
  (let* ((name (generate-file-name image-directory))
         (file (format "%s%s" image-directory name))
