...
(setq-default tab-bar-auto-width-max '(300 15))
#+end_src
...
(global-set-key (kbd "C-c C-c") #'claude-code-transient)
(define-key evil-normal-state-map (kbd "C-z") #'claude-code-transient)
#+end_src
...
#+begin_src emacs-lisp :results none
(defun ediff-visible-buffers ()
  (interactive)
  (let ((windows (window-list)))
    (if (/= (length windows) 2)
        (user-error "Expected exactly 2 visible windows, got %d" (length windows))
      (ediff-buffers (window-buffer (car windows))
...
#+begin_src emacs-lisp :results none
  (use-package org
    ...
    ;; don't ask for confirmation when executing ord-mode functions
    (setq org-link-elisp-confirm-function (lambda (a) t))
...
(use-package org-modern
  ...
  :config
    ...
    (defun org-modern--update-faces-safe (orig-fun &rest args)
      (let ((bg (face-attribute 'default :background nil t)))
        (when (and bg (not (eq bg 'unspecified)))
          (apply orig-fun args))))
    (advice-add 'org-modern--update-faces :around #'org-modern--update-faces-safe)
...
(defvar +org-roam/vaults
  '(("default" . "~/Dropbox/org-roam")
    ("masters" . "~/Library/CloudStorage/Dropbox/masters-roam"))
  "Alist of org-roam vault names to directories.")

(defun +org-roam/switch-vault ()
  "Switch org-roam directory between configured vaults."
  (interactive)
  (let* ((current (abbreviate-file-name (expand-file-name org-roam-directory)))
         (name (completing-read
                (format "Switch org-roam vault (current: %s): " current)
                (mapcar #'car +org-roam/vaults)
                nil t))
         (dir (alist-get name +org-roam/vaults nil nil #'string=)))
    (setq org-roam-directory dir)
    (org-roam-db-sync)
    (message "Switched org-roam vault to: %s (%s)" name dir)))
#+end_src
...
(defvar +org-roam/vaults
  '(("default"  . "~/Dropbox/org-roam")
    ("masters"  . "~/Dropbox/masters-roam")))

(defun +org-roam/switch-vault ()
  (interactive)
  (let* ((current org-roam-directory)
         (choice (completing-read
                  (format "Switch org-roam vault (current: %s): " current)
                  (mapcar #'car +org-roam/vaults)
                  nil t))
         (dir (alist-get choice +org-roam/vaults nil nil #'string=)))
    (setq org-roam-directory (expand-file-name dir))
    (org-roam-db-sync)
    (message "Switched org-roam vault to: %s (%s)" choice dir)))
#+end_src
...
(use-package projectile 
  ...
  :config
    ...
    (setq projectile-project-root-files (remove "Makefile" projectile-project-root-files))
    (setq projectile-project-root-files-bottom-up '("build.sbt" "requirements.txt" "mypy.ini" "pyproject.toml"))
...
(defun magit-create-branch-from-master (name)
  (interactive "sName: ")
  (magit-branch-and-checkout name "master"))
...
(unless (facep 'nano-strong)
  (defface nano-strong '((t :weight bold)) "Nano strong face."))
(unless (facep 'nano-faded)
  (defface nano-faded '((t :foreground "#999999")) "Nano faded face."))
...
(use-package mood-line
  :straight t
  :config
    (advice-add 'mood-line--process-segments :filter-return
                (lambda (result) (or result "")))
    (mood-line-mode)
  :custom
...
(use-package emojify
  ...
  :bind (:map global-map
   (
    ...
    ("C-c SPC o" . (lambda () (interactive) (insert "🟠")))
    ("C-c SPC f" . (lambda () (interactive) (insert "🟣")))
    ("C-c SPC b" . (lambda () (interactive) (insert "🔵")))
...
(use-package lsp-metals
  :straight t
  :custom
    (lsp-metals-server-command "~/dotfiles/scripts/metals-v2")
    (lsp-metals-server-args '("-J-Dmetals.startMcpServer=true"
...
(defun +lsp/get-errors-for-buffer ()
  (mapcar (lambda (el) (plist-get el :message))
...
#+begin_src emacs-lisp :results none
(defun +scala/smart-compile (full-command bl-command)
  (interactive)
  (if (string-match-p "creator/creator-genai-service" default-directory)
      (let* ((genai-service-dir (if (string-match "\\(.*creator/creator-genai-service\\)" default-directory)
                                    (match-string 1 default-directory)
                                  (error "Could not find creator-genai-service directory")))
             (default-directory genai-service-dir)
             (projects-output (shell-command-to-string "bloop projects"))
             (projects (split-string projects-output "\n" t))
             (selected-module (completing-read "Select module: " projects)))
        (+projectile/compile (concat full-command " " selected-module)))
    (+projectile/compile bl-command)))
#+end_src
...
#+begin_src emacs-lisp :results none :tangle no
;; Disabled: incompatible with Emacs 31 built-in treesit. Use treesit-auto instead.
...
#+begin_src emacs-lisp :results none
(defvar my/olivetti-left-offset 10
  "Extra columns to add to the left margin when olivetti-mode is active.")

(defun my/olivetti-asymmetric-margins (orig-fun &rest args)
  "Add offset to left margin after olivetti sets margins."
  (apply orig-fun args)
  (when olivetti-mode
    (set-window-margins (selected-window)
                        (+ (or (car (window-margins)) 0) my/olivetti-left-offset)
                        (max 0 (- (or (cdr (window-margins)) 0) my/olivetti-left-offset)))))

(advice-add 'olivetti-set-window :around #'my/olivetti-asymmetric-margins)
#+end_src
...
(use-package general
  ...
  (leader-def
    ...
    "f o" (lambda () (interactive)(find-file "~/Dropbox/org/todo/daily.org"))
    ...
    "f j" (lambda () (interactive)(find-file "~/Dropbox/org-roam/2023-11-17--22-19-24Z--japanese_learning.org"))
    ...
    "k R" #'koko-replay
    "k q" #'koko-stop
    "k k" #'consult-yank-from-kill-ring
    ...
    "y c" #'claude/copy-claude-code-path
  ...
  (local-leader-def
    ...
    "c c" (lambda () (interactive) (+scala/smart-compile "bloop compile" "bl"))
    "c n" (lambda () (interactive) (+scala/smart-compile "bloop compile --incremental=false" "bl -n"))
    "c r" (lambda () (interactive) (+scala/smart-compile "bloop run" "bl -r"))
    "c t" (lambda () (interactive) (+scala/smart-compile "bloop test" "bl -t"))
...
(use-package claude-code
  :straight (:type git :host github :repo "stevemolitor/claude-code.el" :branch "main" :files ("*.el" (:exclude "demo.gif")))
  :bind-keymap
    ("C-c u" . claude-code-command-map)
  :config
  (setq claude-code-program "pclaude")
  (setq claude-code-program-switches '("--dangerously-skip-permissions"))
  (claude-code-mode))
#+end_src

#+begin_src emacs-lisp :results none
(defun claude/copy-claude-code-path ()
  (interactive)
  (let* ((current-file (buffer-file-name))
         (project-root (projectile-project-root))
         (relative-path (if (and current-file project-root)
                            (file-relative-name current-file project-root)
                          current-file)))
    (kill-new (concat "@" relative-path))))
#+end_src
  ...
  :config 
    ...
    (setq agent-shell-anthropic-claude-environment
	   ...
	   "ANTHROPIC_MODEL" "claude-opus-4-6"
           "ANTHROPIC_VERTEX_PROJECT_ID" "zowie-product-tools"
           "CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC" "1"))

    (setq agent-shell-opencode-acp-command '("oc" "acp"))
    (setq agent-shell-opencode-default-model-id "github-copilot/gemini-3.1-pro-preview")
...
(defun +scala/test ()
  ...
  (let* ((default-directory (projectile-project-root))
	 ...
	 (genai-service-dir (if (string-match "\\(.*creator/creator-genai-service\\)" default-directory)
                                (match-string 1 default-directory)
                              default-directory))
         (default-directory genai-service-dir)
         (projects-output (shell-command-to-string "bloop projects"))
         (projects (split-string projects-output "\n" t))
         (project (completing-read "Select module: " projects))
...
#+begin_src emacs-lisp :results none
(use-package koko
  :load-path "~/.emacs.d/packages/koko/"
  :defer t
  :commands (koko-speak koko-pause koko-resume koko-replay koko-stop)
  :config
  (setq koko-speed 1.3))
#+end_src
...
(defun vscode-open ()
  ...
  (let ((args))
	...
	(when buffer-file-name
	  (push "-g" args)
	  (push (format "%s:%d" buffer-file-name (line-number-at-pos)) args))))

    (push (projectile-project-root) args)

    (apply 'call-process "code" nil 0 nil (nreverse args))))
#+end_src
...
(defun cursor-open ()
  ...
  (let ((args))
    ...
    (apply 'call-process "cursor" nil 0 nil (nreverse args))))
#+end_src
