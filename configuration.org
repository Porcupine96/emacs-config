...
#+begin_src emacs-lisp :results none
(global-set-key (kbd "C-c w w") #'psession-restore-winconf)
(global-set-key (kbd "C-c w s") #'psession-save-winconf)
(global-set-key (kbd "C-c w d") #'psession-delete-winconf)
#+end_src
...
(defun +scala/resolve-imports ()
  ...
  (let* ((path (projectile-project-root))
	 (executable "/Users/lukaszkazmierczak/projects/scala-importer/scala-importer")
	 (output (shell-command-to-string (format (s-concat executable " %s") path)))
         (available (-filter (lambda (s) (not (equal (s-trim s) ""))) (s-split "\n" output)))
	 (missing
	  (-distinct 
	   (-map (lambda (s) (s-replace "not found: type " "" s))
		 (-filter (lambda (s) (s-starts-with? "not found: type" s))
			  (+lsp/get-errors-for-buffer)))))
	 (imports
	  (-filter (lambda (s) (-any? (lambda (m) (s-ends-with? m s)) missing))
		   available)))
...
#+begin_src emacs-lisp :results none
(use-package memo
  :load-path "~/.emacs.default/packages/memo/"
  :defer t
...
#+begin_src emacs-lisp :results none
(use-package winman
  :load-path "~/.emacs.default/packages/winman/"
  :defer t
  :commands (winman-save winman-load winman-delete))
#+end_src
