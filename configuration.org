...
(use-package org
  ...
  (setq org-format-latex-options 
    (plist-put (plist-put org-format-latex-options :background "#202020") :scale 2.0))
...
(defun +lsp/fill-signature--handle (signature)
  (let* ((signatures (gethash "signatures" signature))
         ...
         (label-names (mapcar (lambda (l) (car (s-split ":" l))) labels)))

    (insert (s-join "\n" (mapcar (lambda (n) (s-concat n " = ???,")) label-names)))))
