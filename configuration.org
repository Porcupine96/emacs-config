...
Font.
#+begin_src emacs-lisp :results none
(set-face-attribute 'default nil :font "Input Mono Narrow-11")
...
(defhydra hydra-file (:color blue)
  ...
  ("x" (lambda () (interactive)(find-file "~/.config/i3/config"))))
  #+end_src
...
(defhydra hydra-open (:color blue)
   ...
   _P_ Eshell Open In Project Root
   _o_ Olivetti
   ^^        
   ^^        
   "
...
(use-package helm-bibtex
  ...
  :config
    ...
    ;; (setq bibtex-completion-library-path '("~/Dropbox/papers/pdfs"))
...
(use-package org
  ...
  '((sequence "REPEAT(r)" "TODO(t)" "NEXT(n)" "PROJECT(p)" "REVIEW(R)" "STRT(s)" "SOMEDAY(S)" "WAIT(w)" "|" "DONE(d!)" "KILL(k)")
    (sequence "[ ](T)" "[-](S)" "[?](W)" "|" "[X](D)")))
  ...
  :config
    (setq org-super-agenda-groups
      '(
        ...
        (:name ":flame: Hot" :priority "A" )
        (:name "♨ Warm" :priority "B" )
        (:name "Studies" :priority "B" )
...
(use-package flyspell
  ...
  :bind
     (:map flyspell-mode-map
        ("C-c $" . nil)))
#+end_src
* KILL Configure =guess-language=
CLOSED: [2020-11-01 Sun 00:03]
#+begin_src emacs-lisp :results none
...
(use-package bufler
  ...
  :config 
    (evil-collection-define-key 'key 'normal 
       'bufler-list-mode-map "gp" #'bufler-list-buffer-peek))
#+end_src
...
#+begin_src emacs-lisp :results none
(defun +company/company-complete ()
  "The same as 'company-complete' but adds 'company-select-next' at the end"
  (interactive)
...
(use-package eshell
  :defer t
  :config
    ;; NOTE: the bindings need to be set using a hook
    ;; because of the known issue: https://github.com/noctuid/general.el/issues/32
    (add-hook 'eshell-mode-hook
      (lambda () (define-key eshell-mode-map (kbd "C-l") #'+eshell/clear))))
#+end_src
...
;;   (apply 'start-process
;; 	 (append
;; 	  (list "scrot" "*screenshot*" "scrot" "-s" path))))
#+end_src
