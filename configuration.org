...
(defvar +work/consult-monorepo-last-glob "")

(defun +work/consult-monorepo-glob ()
  (interactive)
  (let* ((glob (read-string "glob: " +work/consult-monorepo-last-glob))
...
Use =evil-snipe= - do I use it? :thinking:
#+begin_src emacs-lisp :results none
...
(use-package jupyter
 ...
 :straight (:no-native-compile t)
 :config
   (load-file "~/.emacs.default/straight/repos/emacs-jupyter/jupyter-channel.el"))
#+end_src
...
(use-package consult
  :ensure t
  :defer t
  :commands (consult-find consult-line +consult/line)
  :init
    (defun +consult/line ()
      (interactive)
      (call-interactively #'consult-line)
      (setq isearch-string (car consult--line-history)))
...
(defun +work/goto-proto ()
  ...
  (let* ((project-name (projectile-project-name))
         ...
         (proto-name (if t/is-plugin
           (concat "plugins/" (string-join (cddr t/chunks) "-"))
           (string-join (cdr t/chunks) "_")))
...
* My packages
#+begin_src emacs-lisp :results none
;; (load-file "~/.emacs.default/packages/pagenda.el")
#+end_src
...
(use-package pscreenshot
  ...
  :defer t
  :commands pscreenshot/org-screenshot-take)
#+end_src
...
(defun writer ()
  ...
  (let* ((content (with-temp-buffer 
		    (insert-file-contents "~/.emacs.default/writer/polish_phrases")
		    (buffer-string)))
	 (phrases (s-split "\n" content))
