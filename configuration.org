...
(defun school/next-class ()
  ...
  (let* ((subject (car (s-split " - " (+org/property-value "TITLE"))))
         ...
         (count (-reduce '+ (org-element-map (org-element-parse-buffer) 'headline
                              (lambda (h)
                                (if (equal "Ćwiczenia" (org-element-property :raw-value
                                                                             (org-element-property :parent h)))
                                    1 0)))))
         (title (concat subject " - ćwiczenia " (number-to-string (+ 1 count)) " (" date ")")))
         ...
         (remove nil
                 (org-map-entries (lambda ()
                                    (let ((elem (org-element-at-point)))
                                      (if (equal "Ćwiczenia" (s-trim (org-element-property :raw-value elem)))
                                          (progn
